...
def build_faiss_index_flat(embeddings):
    dimension = len(embeddings[0])
    
    # Use IndexIDMap with IndexFlatIP for incremental updates (supports add_with_ids and remove_ids)
    # IndexFlatIP uses METRIC_INNER_PRODUCT by default
    ...
    # batched add with IDs (IDs are sequential indices)
    for i in tqdm(range(0, X.shape[0], 32)):
        batch_end = min(i + 32, X.shape[0])
        ids = np.arange(i, batch_end, dtype=np.int64)
        faiss_index.add_with_ids(X[i:batch_end], ids)
    return faiss_index
