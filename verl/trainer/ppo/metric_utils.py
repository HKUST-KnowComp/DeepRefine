...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # When response_mask is all zeros (e.g. only action_generation counts for loss but batch
    # has no action tokens), valid_adv/valid_returns/valid_values are empty. Avoid torch.max/min
    # on empty tensors by using placeholder stats.
    _empty_adv = valid_adv.numel() == 0
    if _empty_adv:
        adv_mean, adv_max, adv_min = 0.0, 0.0, 0.0
        ret_mean, ret_max, ret_min = 0.0, 0.0, 0.0
        val_mean, val_max, val_min = 0.0, 0.0, 0.0
        ...
        ret_mean = torch.mean(valid_returns).detach().item()
        ret_max = torch.max(valid_returns).detach().item()
        ret_min = torch.min(valid_returns).detach().item()
        if use_critic:
            val_mean = torch.mean(valid_values).detach().item()
            val_max = torch.max(valid_values).detach().item()
            val_min = torch.min(valid_values).detach().item()
