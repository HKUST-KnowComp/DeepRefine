...
import math
from autorefiner.src.rag_server.tog_prompt import REASONING_PROMPT, ANSWER_GENERATION_PROMPT, FEW_SHOT_EXAMPLE, VERIFY_ANSWER_PROMPT
from autorefiner.src.rag_server.deeprefine_prompt import REFINE_SUBGRAPH_SYSTEM_PROMPT, REFINE_SUBGRAPH_USER_PROMPT
import jellyfish
import logging 
...
class SubgraphRetriever(BaseRetriever):
    ...
    async def ner(self, text):
        ...
        try:
            ...
            # disable structured output for NER
            sampling_params_backup["response_format"] = None
            response = await self.llm_generator.generate_response(messages, **sampling_params_backup)
    ...
    async def index_kg(self, kg: DiGraph, batch_size:int = 100):
        """
        Obtain the node and triple embeddings for the knowledge graph.
        Args:
            kg (DiGraph): The knowledge graph to index.
    ...
    async def retrieve_topk_nodes(self, query):
        ...
        Args:
            query (str): The query to retrieve top-k nodes from.
        Returns:
            list: A list of top-k nodes.
        """
        is_query_only = False
    ...
    async def construct_subgraph(self, query, initial_nodes):
        """
        Construct (Sample) a multi-hop subgraph around initial nodes up to self.num_hop.
        """
        subgraph = DiGraph()
    ...
    async def retrieve(self, question, kg: DiGraph, sampling_params: dict, **kwargs) -> str:
        ...
        self.reward_function = kwargs.get("reward_function", "gbd_reward")
        ...
        elif self.reward_function == "gbd_reward" or self.reward_function == "gbd_f1_reward":
            self.answer_gen_fn = self.generate_answer
        elif self.reward_function not in ['f1_reward', 'deduce_reward', 'gbd_reward', 'gbd_f1_reward']:
            raise ValueError(f"reward_function {self.reward_function} not supported")
        self.KG = kg
        self.sampling_params = sampling_params

        if kg is None or len(kg.edges) == 0:
            logging.warning("SubgraphRetriever: empty KG edges, return empty answer directly.")
            return json.dumps({"answer": "", "edge_coverage": 0.0, "semantic_reward": 0.0})

        # Encoding (Indexing) the full KG
        ...
        if self.config.use_full_kg:
            ...
            # sample a query-oriented subgraph from full KG
        ...
        answer = await self.answer_gen_fn(question, subgraph)
    ...
    async def generate_answer(self, query, subgraph: DiGraph, **kwargs):
        ...
        triples_string = ". ".join([f"({self.KG.nodes[s]['id']}-{r}->{self.KG.nodes[o]['id']})" for s, r, o in triples])
        ...
        sampling_params_backup = {**self.sampling_params}
        # disable structured output for free-form answer generation
        sampling_params_backup["response_format"] = None
        messages.append({"role": "user", "content": f"{triples_string}\n\n{query}"})
        generated_text = await self.llm_generator.generate_response(messages, **sampling_params_backup)
    ...
    async def deduce_answer(self, query, subgraph: DiGraph, answer, **kwargs):
        """
        Given a question and a retrieved subgraph, LLM Judge if the question is answerable
        """
        triples = [(u, d["relation"], v) for u, v, d in subgraph.edges(data=True)]
